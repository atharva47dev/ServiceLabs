...
const deleteTickets = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(Tickets,filter);
    return response;
  } catch (error){
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const ticketsFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const ticketsCnt = await dbService.deleteMany(Tickets,ticketsFilter);
...
const countTickets = async (filter) =>{
  try {
    const ticketsCnt =  await dbService.count(Tickets,filter);
    return { tickets : ticketsCnt };
  } catch (error){
    throw new Error(error.message);
  }
};
...
const countUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const ticketsFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const ticketsCnt =  await dbService.count(Tickets,ticketsFilter);
...
const softDeleteTickets = async (filter,updateBody) =>{  
  try {
    const ticketsCnt =  await dbService.updateMany(Tickets,filter);
    return { tickets : ticketsCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const ticketsFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const ticketsCnt = await dbService.updateMany(Tickets,ticketsFilter,updateBody);
